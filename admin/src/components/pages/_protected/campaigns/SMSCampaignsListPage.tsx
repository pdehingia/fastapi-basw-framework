...
import type { SMSCampaign, SMSCampaignFilters, CampaignStatus, CampaignType, TargetAudience } from '@/types/api.types';

export function SMSCampaignsListPage() {
  ...
  const { data: campaignsData, isLoading, refetch } = useQuery({
    queryKey: ['campaigns', 'sms', 'list', filters, page],
    queryFn: async () => {
      const response = await campaignsService.getSMSCampaigns(filters, { page, size: 50 });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (campaignId: string) => {
      return campaignsService.deleteSMSCampaign(campaignId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['campaigns', 'sms'] });
    },
  });
  ...
  const sendMutation = useMutation({
    mutationFn: async (campaignId: string) => {
      return campaignsService.sendSMSCampaign(campaignId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['campaigns', 'sms'] });
    },
  ...
  const handleFilterChange = (key: keyof SMSCampaignFilters, value: any) => {
  ...
  const handleExport = async () => {
    ...
    console.log('Export SMS campaigns with filters:', filters);
  };
  ...
  const columns = [
      ...
      render: (campaign: SMSCampaign) => (
        <div>
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1">
            {campaign.message_content.substring(0, 50)}...
          </div>
        </div>
      ),
    },
    {
      key: 'campaign_type',
      label: 'Type',
      render: (campaign: SMSCampaign) => getTypeBadge(campaign.campaign_type),
    },
    {
      key: 'target_audience',
      label: 'Audience',
      ...
      render: (campaign: SMSCampaign) => (
        <div className="text-sm">
          <div className="text-green-600 dark:text-green-400">
            {campaign.total_delivered.toLocaleString()} delivered
          </div>
          <div className="text-red-600 dark:text-red-400">
            {campaign.total_failed.toLocaleString()} failed
          </div>
        </div>
      ),
    },
    {
      key: 'campaign_status',
      label: 'Status',
      render: (campaign: SMSCampaign) => getStatusBadge(campaign.campaign_status),
    },
    {
      key: 'scheduled_at',
      label: 'Schedule',
      ...
      render: (campaign: SMSCampaign) => (
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate({ to: `/campaigns/sms/${campaign.id}` })}
          >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Manage and monitor SMS marketing campaigns
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => navigate({ to: '/campaigns/sms/create' })}>
